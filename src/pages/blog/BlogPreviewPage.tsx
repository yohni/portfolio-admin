...
export default function BlogPreviewPage() {
  ...
  if (error || !blog) {
    return (
      <div className="page">
        <div className="alert alert--error" style={{ maxWidth: 500 }}>{error || 'Post not found'}</div>
        <div style={{ marginTop: 16 }}>
          <Link to="/blog" className="btn btn--secondary btn--sm">Back to posts</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="preview-page">
      <div className="preview-header">
        ...
        <div style={{ flex: 1 }} />
        <button className="btn btn--secondary btn--sm" onClick={() => navigate(`/blog/${id}/edit`)}>
          <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
